...
def create_prompt(prompt: schemas.PromptBase, status_code=status.HTTP_201_CREATED, db: Session = Depends(database.get_db)):
...
def update_prompt(id: int, updated_prompt: schemas.PromptBase, db: Session = Depends(database.get_db)):
...
@router.delete("/{id}")
def delete_prompt(id: int, db: Session = Depends(database.get_db)):
    prompt_query = db.query(models.Prompt).filter(models.Prompt.id == id)
    prompt = prompt_query.first()
